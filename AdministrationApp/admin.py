from django.contrib import admin
from AdministrationApp.models import PatentRegistration, QuickAppointment, Appointment, Nurse, Gallery, DoctorsProfile, Feedback, Subscribe, Shop, ShopRequest, CounterValues

admin.site.register(PatentRegistration)
admin.site.register(QuickAppointment)
admin.site.register(Appointment)
admin.site.register(Nurse)
admin.site.register(Gallery)
admin.site.register(DoctorsProfile)
admin.site.register(Feedback)
admin.site.register(Subscribe)
admin.site.register(Shop)
admin.site.register(ShopRequest)
admin.site.register(CounterValues)